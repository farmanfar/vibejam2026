...
- **ShopScene:** Star-level combining UI (auto-merge on duplicate buy). Commander display + swap offer every 3 wins (10 credits). Merchant display (random per round, weighted by team). Sell refund: full during shop phase, ceil(cost/2) after battle. 10 credits/round flat income. 5 bench slots from stage 1 (remove team-size scaling). No permadeath — full team persists.
- **BattleScene:** Remove permadeath (dead units come back at full HP next round). Support class synergy effects (armor, splash, double-attack, taunt, heal). Trigger abilities (on-attack, on-hurt, on-faint, start-of-battle).
