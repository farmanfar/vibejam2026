...
export class ShopScene extends Scene {
  ...
  init(data) {
    ...
    this.team = data.team || [];
    this.shop = new ShopManager(WARRIORS, this.round);
    this.shopOffer = this.shop.roll();

    // Clear refs from previous lifecycle (scene.restart reuses the instance)
  ...
  create() {
    ...
    new PixelLabel(this, 12, 8, `ROUND ${this.round}`, { scale: 2, color: 'accent' });
    this.goldLabel = new PixelLabel(this, width / 2, 8, `GOLD: ${this.gold}`, {
      scale: 2, tint: Theme.warning, align: 'center',
    });
    new PixelLabel(this, width - 12, 8, `LIVES: ${this.lives}`, {
    ...
    new PixelButton(this, width / 2 + 130, 475, 'FIGHT!', () => {
      this._startBattle();
  ...
  _reroll() {
    ...
    this.scene.restart({
      round: this.round, gold: this.gold, lives: this.lives, team: this.team,
    });
  }
